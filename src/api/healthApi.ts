...
export class HealthApiClient {
  ...
  static async getActivities(startDate: string, endDate: string): Promise<DailyActivity[]> {
    const [stepsData, distanceData, floorsData, kcalData, azmData] = await Promise.all([
      fetchRollupPoints(DATA_TYPES.steps, startDate, endDate),
      fetchRollupPoints(DATA_TYPES.distance, startDate, endDate),
      fetchRollupPoints(DATA_TYPES.floors, startDate, endDate),
      fetchRollupPoints(DATA_TYPES.totalCalories, startDate, endDate),
      fetchRollupPoints(DATA_TYPES.activeZoneMinutes, startDate, endDate),
    ]);

    const actMap = new Map<string, DailyActivity>();
    const getAct = (d: string) => {
      if (!actMap.has(d)) actMap.set(d, { date: d, steps: 0, distanceKm: 0, floors: 0, caloriesBurned: 0, activeZoneMinutes: 0 });
      return actMap.get(d)!;
    };

    stepsData.forEach((p: any) => {
      const d = civilToDateStr(p.civilStartTime);
      if (d) getAct(d).steps = Math.round(num(p.steps?.countSum));
    });
    distanceData.forEach((p: any) => {
      const d = civilToDateStr(p.civilStartTime);
      if (d) getAct(d).distanceKm = roundTo(num(p.distance?.millimetersSum) / 1000000, 2);
    });
    floorsData.forEach((p: any) => {
      const d = civilToDateStr(p.civilStartTime);
      if (d) getAct(d).floors = Math.round(num(p.floors?.countSum));
    });
    kcalData.forEach((p: any) => {
      const d = civilToDateStr(p.civilStartTime);
      if (d) getAct(d).caloriesBurned = Math.round(num(p.totalCalories?.kcalSum));
    });
    azmData.forEach((p: any) => {
      const d = civilToDateStr(p.civilStartTime);
      if (d) {
        getAct(d).activeZoneMinutes = Math.round(
    ...
    return Array.from(actMap.values()).sort((a, b) => a.date.localeCompare(b.date));
  }
  ...
  static async getHeartRatesBatch(startDate: string, endDate: string): Promise<DailyHeartRate[]> {
    const restingMap = await fetchDailySummary(
      DATA_TYPES.restingHeartRate, "dailyRestingHeartRate", startDate, endDate,
      (u) => Math.round(num(u.beatsPerMinute)),
    );

    const arr: DailyHeartRate[] = [];
    let cur = new Date(`${startDate}T00:00:00Z`);
    const end = new Date(`${endDate}T00:00:00Z`);
    
    // We fetch intraday data for all requested days. To avoid the 60 req/min quota, we add a delay.
    while (cur <= end) {
      const d = cur.toISOString().slice(0, 10);
      let intraday: any[] = [];
      
      try {
        const token = filterToken(DATA_TYPES.heartRate);
        const filter =
          `${token}.sample_time.physical_time >= "${d}T00:00:00Z" AND ` +
          `${token}.sample_time.physical_time < "${nextDay(d)}T00:00:00Z"`;
          ...
          intraday = points
            .map((p: any) => {
              ...
              return { time, bpm: Math.round(num(hr.beatsPerMinute)) };
            })
            .filter(Boolean);
          intraday.sort((a, b) => a.time.localeCompare(b.time));
        } catch (e) {
          console.warn(`Failed to fetch intraday HR for ${d}`);
        }
      
      arr.push({ date: d, restingHeartRate: restingMap.get(d) ?? 0, intraday });
      cur.setUTCDate(cur.getUTCDate() + 1);
      // Wait 1200ms to stay well below 60 req/min
  ...
  static async getNightlyVitals(startDate: string, endDate: string): Promise<NightlyVitals[]> {
    const [spo2, breathing, hrv, temp] = await Promise.all([
      fetchDailySummary(DATA_TYPES.spo2, "dailyOxygenSaturation", startDate, endDate,
        (u) => Math.round(num(u.averagePercentage))),
      fetchDailySummary(DATA_TYPES.breathingRate, "dailyRespiratoryRate", startDate, endDate,
        (u) => Math.round(num(u.breathsPerMinute))),
      fetchDailySummary(DATA_TYPES.hrv, "dailyHeartRateVariability", startDate, endDate,
        (u) => Math.round(num(u.averageHeartRateVariabilityMilliseconds ?? u.deepSleepRootMeanSquareOfSuccessiveDifferencesMilliseconds))),
